...
import shop.retries.{ Retry, Retryable }
...
final case class Checkout[F[_]: Background: Logger: MonadThrow: Retry](
  ...
  private def processPayment(in: Payment): F[PaymentId] =
    Retry[F]
      .retry(policy, Retryable.Payments)(payments.process(in))
      .adaptError {
        case e =>
  ...
  private def createOrder(
    ...
    val action =
      Retry[F]
        .retry(policy, Retryable.Orders)(orders.create(userId, paymentId, items, total))
        .adaptError {
