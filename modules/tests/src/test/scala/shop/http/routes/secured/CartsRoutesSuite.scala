  ...
  test("POST add item to shopping cart succeeds"){
    ...
    forall(gen) {
      case (user, c) =>
        ...
        val routes =
          CartRoutes[IO](new TestShoppingCart).routes(authMiddleware(user))
